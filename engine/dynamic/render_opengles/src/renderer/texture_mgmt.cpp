...
namespace argus {
    void get_or_load_texture(RendererState &state, const Resource &material_res) {
        ...
        if (AGLET_GL_ES_VERSION_3_0) {
            glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, GLsizei(texture.m_width), GLsizei(texture.m_height));
        } else {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, GLsizei(texture.m_width), GLsizei(texture.m_height), 0, GL_RGBA,
                    GL_UNSIGNED_BYTE,
                    nullptr);
        }

        for (uint32_t y = 0; y < texture.m_height; y++) {
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, int32_t(y), GLsizei(texture.m_width), 1, GL_RGBA,
