...
namespace argus {
    ...
    Handle HandleTable::create_handle(void *ptr) {
        ...
        if (dest_chunk == nullptr) {
            ...
            if (!found_empty) {
                first_empty_chunk_index = m_pimpl->chunks.rbegin()->first + 1;
            }
    ...
    void HandleTable::release_handle(Handle handle) {
        ...
        if (chunk->open_indices.size() == CHUNK_SIZE - 1 && m_pimpl->chunks.size() > 1) {
            // just deallocate the chunk
            delete chunk;
            m_pimpl->chunks.erase(chunk_index);
