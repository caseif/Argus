...
unsafe fn copy_rust_map_to_interop_map_str(map: &HashMap<String, String>) -> *const u8 {
    let buf_len = map
        .iter()
        .map(|(k, v)| {
            _next_aligned_val(size_of::<SizedByteArray>() + k.len())
...
#[no_mangle]
pub unsafe extern "C" fn transpile_glsl(
    stages: *const Stage,
    glsl_sources: *const *const c_char,
    count: usize,
    client: Client,
    ...
    let compile_res = match super::shadertools::compile_glsl_to_spirv(
        &sources_map,
        client,
        client_version,
        spirv_version,
    ...
    let res = alloc(Layout::new::<InteropShaderCompilationResult>())
        as *mut InteropShaderCompilationResult;
...
pub unsafe extern "C" fn free_compilation_result(result: *mut InteropShaderCompilationResult) {
    ...
    if shader_count > 0 {
        dealloc(
            (*result).stages as *mut u8,
            match Layout::array::<*const Stage>(shader_count) {
                Ok(layout) => layout,
                Err(msg) => panic!("Failed to create SPIR-V stages array layout: {}", msg),
            },
        );
        ...
        for i in 0..shader_count {
            let bin_struct_ptr =
                *(*result).spirv_binaries.offset(i.try_into().unwrap()) as *mut SizedByteArray;
            let bin_len = (*bin_struct_ptr).size;
            dealloc(
                bin_struct_ptr as *mut u8,
        ...
        dealloc(
            (*result).spirv_binaries as *mut u8,
            match Layout::array::<*const u8>(shader_count) {
                Ok(layout) => layout,
                Err(msg) => panic!("Failed to create SPIR-V binaries array layout: {}", msg),
            },
        );
    ...
    dealloc(
        result as *mut u8,
        Layout::new::<InteropShaderCompilationResult>(),
    );
}
