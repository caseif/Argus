...
function(argus_parse_version_string version_string out_major out_minor out_patch out_tweak)
  string(REPLACE "." ";" version_split "${version_string}")
  list(LENGTH version_split num_components)
  if(NOT "${out_major}" STREQUAL "")
    if(${num_components} GREATER_EQUAL 1)
      list(GET version_split 0 v_major)
    else()
      set(v_major 0)
    endif()
    set(${out_major} ${v_major} PARENT_SCOPE)
  endif()
  if(NOT "${out_major}" STREQUAL "")
    if(${num_components} GREATER_EQUAL 2)
      list(GET version_split 1 v_minor)
    else()
      set(v_minor 0)
    endif()
    set(${out_minor} ${v_minor} PARENT_SCOPE)
  endif()
  if(NOT "${out_major}" STREQUAL "")
    if(${num_components} GREATER_EQUAL 3)
      list(GET version_split 2 v_patch)
    else()
      set(v_patch 0)
    endif()
    set(${out_patch} ${v_patch} PARENT_SCOPE)
  endif()
  if(NOT "${out_major}" STREQUAL "")
    if(${num_components} GREATER_EQUAL 4)
      list(GET version_split 3 v_tweak)
    else()
      set(v_tweak 0)
    endif()
    set(${out_tweak} ${v_tweak} PARENT_SCOPE)
  endif()
endfunction()

function(argus_check_version_greater_equal out_result version min_version)
  argus_parse_version_string(${version} v_major v_minor v_patch v_tweak)
  argus_parse_version_string(${min_version} min_major min_minor min_patch min_tweak)
  if(${v_major} GREATER ${min_major})
    set(${out_result} 1 PARENT_SCOPE)
    return()
  elseif(${v_major} EQUAL ${min_major})
    if(${v_minor} GREATER ${min_minor})
      set(${out_result} 1 PARENT_SCOPE)
      return()
    elseif(${v_minor} EQUAL ${min_minor})
      if(${v_patch} GREATER ${min_patch})
        set(${out_result} 1 PARENT_SCOPE)
        return()
      elseif(${v_patch} EQUAL ${min_patch})
        if(${v_tweak} GREATER_EQUAL ${min_tweak})
          set(${out_result} 1 PARENT_SCOPE)
          return()
        endif()
      endif()
...
function(argus_check_version_less_equal out_result version max_version)
  argus_parse_version_string(${version} v_major v_minor v_patch v_tweak)
  argus_parse_version_string(${max_version} max_major max_minor max_patch max_tweak)

  if(${v_major} LESS ${max_major})
    set(${out_result} 1 PARENT_SCOPE)
    return()
  elseif(${v_major} EQUAL ${max_major})
    if(${v_minor} LESS ${max_minor})
      set(${out_result} 1 PARENT_SCOPE)
      return()
    elseif(${v_minor} EQUAL ${max_minor})
      if(${v_patch} LESS ${max_patch})
        set(${out_result} 1 PARENT_SCOPE)
        return()
      elseif(${v_patch} EQUAL ${max_patch})
        if(${v_tweak} LESS_EQUAL ${max_tweak})
          set(${out_result} 1 PARENT_SCOPE)
          return()
        endif()
      endif()
...
function(argus_check_version_in_range out_result version min_incl max_incl)
  argus_check_version_greater_equal(above_min ${version} ${min_incl})
  argus_check_version_less_equal(below_max ${version} ${max_incl})
